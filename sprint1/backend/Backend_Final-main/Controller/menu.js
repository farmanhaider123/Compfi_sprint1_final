...
exports.toggleModuleStatus = async (req, res) => {
  ...
  let Isexist = await Module.findOneAndUpdate({ _id: req.body.userid }, { $set: { status } })
  if (Isexist) {
    res.send({ "err": 0, "msg": "Module Status is updated" })
  }
  if (!Isexist) {
    res.send({ "err": 1, "msg": "Module is not exist" })
  }
}